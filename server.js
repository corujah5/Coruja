...
app.post('/api/me/state',(req,res)=>{const u=authUser(req);if(!u)return res.status(401).json({error:'Não autenticado.'});const users=readUsers(); const clean=cleanState(req.body?.state); writeUserState(u,clean); if(req.body?.name)u.name=cleanText(req.body.name,60); writeUsers(users); res.json({ok:true,state:clean});});
// Salvamento para celular/fechamento de aba: sendBeacon não permite cabeçalho Authorization,
// então aceitamos o token no corpo apenas neste endpoint e validamos a mesma sessão.
app.post('/api/me/state-beacon',(req,res)=>{const token=String(req.body?.token||'').trim();if(!token)return res.status(401).json({error:'Não autenticado.'});const sessions=readSessions();const userId=sessions[token];if(!userId)return res.status(401).json({error:'Sessão inválida.'});const users=readUsers();const u=users.find(x=>x.id===userId);if(!u||u.blocked)return res.status(401).json({error:'Não autenticado.'});const clean=cleanState(req.body?.state);writeUserState(u,clean);if(req.body?.name)u.name=cleanText(req.body.name,60);writeUsers(users);res.json({ok:true});});
